...
The camera is the "good camera""" with the zoom and focus functions.
The joystick functionality is as discussed in the ppt. 
...
while not done:
    ...
    for event in pygame.event.get():
        ...
        try :
            ...
            print(Fore.RED + f' ← Receive: [ERROR] in Controller. Battery level {joystick.get_power_level()}.Connect to charge or change to wired mode '+ Style.RESET_ALL )
    ...
    if X!=0 or Y != 0:
        M.big_move(X,Y)
        # print(Fore.BLUE + f' ← Received: jogging X:{X} Y:{Y} '+ Style.RESET_ALL )
