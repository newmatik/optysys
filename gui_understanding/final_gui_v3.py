...
'''
The following UI displays images from 2 cameras: OpenCV camera and webcam of the laptop. 
This version shows exactly as required. 
...
class App(QWidget):
    def __init__(self):
        ...
        self.zoom_overview_imglabel = QLabel(self)
        self.zoom_overview_imglabel.resize(2,2)

        self.disp2_thread = VideoThread2()
        # connect its signal to the update_image slot
        self.disp2_thread.change_pixmap_signal.connect(self.update_image_disp2)
        # start the thread
        self.disp2_thread.start()

        # zoom_overview_image = QPixmap('images/overview.jpg')
        # zoom_overview_imglabel.setPixmap(zoom_overview_image)
        zoom_overivew_layout.addWidget(self.zoom_overview_imglabel)
    ...
    @pyqtSlot(np.ndarray)
    def update_image_disp2(self, cv_img):
        """Updates the image_label with a new opencv image"""
        qt_img = self.convert_cv_qt_disp2(cv_img)
        self.zoom_overview_imglabel.setPixmap(qt_img)
    
    def convert_cv_qt_disp2(self, cv_img):
        """Convert from an opencv image to QPixmap"""
        rgb_image = cv2.cvtColor(cv_img, cv2.COLOR_BGR2RGB)
        h, w, ch = rgb_image.shape
        bytes_per_line = ch * w
